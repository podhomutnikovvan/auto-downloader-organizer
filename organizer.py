...
# -*- coding: utf-8 -*-
"""
Downloader Organizer — авто-организатор папки «Загрузки» (один файл).

Как это работает:
  1. Браузер продолжает скачивать в свою папку по умолчанию — ничего менять не надо.
  ...
  3. Когда файл ДОКАЧАН (исчезло расширение .part/.crdownload или размер перестал
     меняться несколько секунд), он ПЕРЕМЕЩАЕТСЯ в подпапку по типу файла:
     Документы / Таблицы / Картинки / Музыка / Видео / Архивы / Установщики / Код / Прочее.
  4. Все перемещения пишутся в журнал history.log рядом с этим файлом.

Запуск:      python organizer.py          (остановка — Ctrl+C)
Проверка правил без переноса:  python organizer.py --dry-run photo.jpg archive.zip
Отмена последнего переноса:    python organizer.py --undo

Настройки меняются прямо в словаре CONFIG ниже. Если рядом создать config.json —
его значения переопределят CONFIG (удобно, чтобы не трогать код).
"""
...
CONFIG_FILE = BASE / "config.json"          # необязательный внешний конфиг
...
def move_file(src: Path, cfg: dict) -> Path | None:
    ...
    log.info("✔ %s  →  %s%s%s", src.name, folder, os.sep, dst.name)
    return dst
...
def main():
    ...
    log.info("Раскладываю в:    %s%s<категория>", cfg["dest_dir"], os.sep)
