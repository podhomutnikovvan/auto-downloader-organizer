...
# -*- coding: utf-8 -*-
"""
Downloader Organizer — авто-организатор папки «Загрузки».

Как это работает:
  1. Браузер (по умолчанию) продолжает качать файлы в ~/Downloads — ничего менять не надо.
  2. Программа следит за этой папкой "вживую" (watchdog), почти не нагружая CPU.
  3. Когда файл ДОКАЧАН (исчезло расширение .part/.crdownload или размер
     перестал меняться N секунд), он перемещается в подпапку по правилам из config.json.
  4. Все перемещения пишутся в журнал history.log, есть команда отмены (--undo).

Запуск:            python3 downloader_organizer.py
Проверка правил:   python3 downloader_organizer.py --dry-run <файл...>
Отмена последнего: python3 downloader_organizer.py --undo
Остановка:         Ctrl+C
"""
...
DEFAULT_CONFIG = {
    # Путь к папке загрузок. "~" будет раскрыт автоматически.
    "downloads_dir": "~/Downloads",
    # Сколько секунд размер файла не должен меняться, чтобы считать его докачанным
    "settle_seconds": 3,
    # Расширения, которые означают "файл ещё качается" — их не трогаем
    "partial_extensions": [".part", ".crdownload", ".download", ".tmp"],
    # Правила: расширение -> имя папки. Всё неизвестное goes в "Прочее"
    "rules": {
        ".pdf": "Документы", ".docx": "Документы", ".txt": "Документы", ".md": "Документы",
        ".xlsx": "Таблицы", ".csv": "Таблицы",
        ".jpg": "Картинки", ".jpeg": "Картинки", ".png": "Картинки", ".gif": "Картинки", ".webp": "Картинки",
        ".mp3": "Музыка", ".flac": "Музыка",
        ".mp4": "Видео", ".mkv": "Видео", ".avi": "Видео",
        ".zip": "Архивы", ".rar": "Архивы", ".7z": "Архивы", ".tar": "Архивы", ".gz": "Архивы",
        ".exe": "Установщики", ".msi": "Установщики", ".dmg": "Установщики", ".apk": "Установщики",
...
def load_config() -> dict:
    if not CONFIG_FILE.exists():
        CONFIG_FILE.write_text(json.dumps(DEFAULT_CONFIG, ensure_ascii=False, indent=2),
                               encoding="utf-8")
        log.info("Создан файл настроек %s — можете отредактировать правила.", CONFIG_FILE)
...
def move_file(src: Path, cfg: dict) -> Path | None:
    ...
    dst_dir = Path(cfg["downloads_dir"]).expanduser() / folder
...
class DownloadWatcher(FileSystemEventHandler):
    ...
    def __init__(self, cfg: dict):
        self.cfg = cfg
        self.pending: dict[Path, float] = {}          # файл -> время последней записи
    ...
    def on_moved(self, event):
        ...
        if not event.is_directory:
            ...
            if not self._looks_partial(dest) and dest.parent == Path(self.cfg["downloads_dir"]).expanduser():
                self.pending.pop(Path(event.src_path), None)
                self._finish(dest)

    def _touch(self, path: Path):
        dl = Path(self.cfg["downloads_dir"]).expanduser()
        if path.parent != dl or self._looks_partial(path):
            return
    ...
    def sweep(self):
        """Периодическая проверка: файлы, молчащие дольше settle_seconds, считаются готовыми."""
        now = time.time()
...
def undo_last():
    ...
    if dst.exists():
        ...
        log.info("↩ Возвращён файл: %s", src.name)
...
def main():
    ...
    cfg = load_config()
    dl = Path(cfg["downloads_dir"]).expanduser()

    # Режимы разовой проверки
    if "--dry-run" in sys.argv:
        for name in sys.argv[sys.argv.index("--dry-run") + 1:]:
            print(f"{name:30s} → {classify(name, cfg)}")
        return
    if "--undo" in sys.argv:
        undo_last()
        return
    ...
    log.info("Слежу за папкой: %s", dl)
    log.info("Правила: %s (редактируются в %s)", "config.json", CONFIG_FILE.name)
    log.info("Остановка: Ctrl+C\n")

    watcher = DownloadWatcher(cfg)
    observer = Observer()
    observer.schedule(watcher, str(dl), recursive=False)
    observer.start()
    try:
        while True:
            watcher.sweep()
